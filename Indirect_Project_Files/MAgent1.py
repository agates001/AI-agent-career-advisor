...
JobHunter = Agent(
    ...
    # Limits the maximum number of thought-action-observation loops an agent can perform 
    # on a single task.Autonomous agents use a loop to reason, call tools, check results, 
    # and decide if they are done. If an agent gets confused or stuck trying to 
    # fix a tool error, it can loop indefinitely.
    max_rpm=10       
    ...
    #Sets an upper bound on the number of API requests per minute the agent 
    # can send to the Large Language Model (LLM). When multiple agents run concurrently 
    # or execute heavy tool use, they can flood your LLM provider with traffic. 
    # Setting max_rpm helps prevent you from hitting provider rate limits 
    # (RateLimitError) and helps manage costs by pacing out calls.
...
job_write_task = Task(
    ...
    agent=JobLister,
    output_file="ai_agents_job_report5.md"
)
